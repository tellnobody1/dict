\input{dict}

\begin{document}
\title{\uppercase{Англо-\\український\\словник\\сучасних\\слів}}
\author{@tellnobody1}
\maketitle

\thispagestyle{empty}
\clearpage

\begin{center}
\section{\uppercase{Англійський алфавіт}}
\begin{tabular}{| c c c c c c c |}
  \hline
  Aa & Ee & Ii & Mm & Qq & Uu & Yy \\
  Bb & Ff & Jj & Nn & Rr & Vv & Zz \\
  Cc & Gg & Kk & Oo & Ss & Ww &    \\
  Dd & Hh & Ll & Pp & Tt & Xx &    \\
  \hline
\end{tabular}
\end{center}

\thispagestyle{empty}
\clearpage

\begin{twocolumn}

\section{@}
\entry{@}{равлик}

\section{A}
\entry{accessory}{причанда́л}\\
\entry{account}{обліковий запис}\\
\entry{action role-playing game}{рольовий бойовик}\\
\entry{add-on}{додаток}\\
\entry{anime}{японська анімація, аніме}\\
\entry{app}{див. \hyperlink{app}{application}}\\
\hypertarget{app}{\entry{application}{застосунок, програма}}\\
\entry{assistant}{помі́чни́к, пома\-га́ч}

\section{B}
\entry{backpressure, back pressure}{проти\-ти́ск, зворо́тний тиск, контрти́ск}\\
\entry{barrier}{рога́тка, зворотниця, заста́ва, пере́пи́н, шлаг\-ба́\-ум}\\
\entry{benchmark}{контро́льне завда́ння́}\\
\entry{browser}{переглядач}\\
\entry{byte}{байт (беззна́кове ціле число від 0 до $2^8$-1 включно)}

\section{C}
\entry{caffeine}{кавеї́н (-ну)}\\
\entry{compilation}{збірка, збирати}\\
\entry{compile}{збирати}\\
\entry{creator}{творе́ць}\\
\entry{CSS [Cascading Style Sheets]}{стилі}\\
\entry{cybernetics}{кіберне́тика}\\
\entry{cyberpunk}{кіберпа́нщина}

\section{D}
\entry{deadline}{речене́ць}\\
\entry{default}{типовий}\\
\subentry{by \textasciitilde}{типово}\\
\entry{delivery}{дові́з (р. -во́зу)}\\
\entry{developer}{розро́бник}\\
\entry{device}{пристрі́й, при́лад}\\
\entry{dislike}{невподоба}\\
\entry{driver}{керун}

\section{E}
\entry{easter egg}{яйце́-райце́, прихованка}\\
\entry{engine}{руші́й}

\section{F}
\entry{fantasy}{химе́рство}\\
\entry{FOSS [Free and Open-Source Software]}{ВВПЗ [вільне та відкрите програмне забезпечення]}\\
\entry{framework}{кістя́к}

\section{G}
\entry{graceful shutdown}{ґре́чна при́пинка}

\section{H}
\entry{handler}{\textbf{1.} маніпуля́тор; при́ст\-рій керува́ння, керува́ч \textbf{2.} опрацьо́вувач, програ́ма опра\-цьо́\-вування}\\
\hypertarget{eventhandler}{\subentry{event \textasciitilde}{програ́ма опра\-цьо́\-вування [опрацьо́вувач] поді́й}}\\
\entry{help}{по́міч}\\
\entry{host}{відник}

\section{L}
\entry{listener}{\textbf{1.} слухач \textbf{2.} (в мовах Java, JavaScript) те саме, що \hyperlink{eventhandler}{event handler}}\\
\entry{like}{вподобайка}

\section{M}
\entry{manga}{японські мальописи, манга}\\
\entry{menu}{\textbf{1.} спис страв, стравоспис, меню, карта (страв) \textbf{2.} (перен.) спис, меню}\\
\entry{message}{повідомлення}

\section{P}
\entry{path}{путь (ж. р.)}\\
\entry{phone}{див. \hyperlink{telephone}{telephone}}\\
\entry{police}{полі́ція}\\
\subentry{\textasciitilde\ state}{поліці́йна держа́ва}\\
\entry{policeman}{поліція́н(т)}\\
\entry{postapocalypse}{постапо\-ка́\-ліпсис}\\
\entry{problem}{за(в)да́ча}\\
\entry{project}{проє́кт}

\section{Q}
\entry{queue}{че́рга́}

\section{R}
\entry{real-time strategy}{страте́гія в реа́льному ча́сі}\\
\entry{real-time tactics}{тактика в реальному часі}\\
\entry{review}{о́гляд}\\
\hypertarget{rpg}{\entry{role-playing video game}{рольова відеогра}}\\
\entry{RPG}{див. \hyperlink{rpg}{role-playing game}}

\section{S}
\entry{screenshot}{зняток}\\
\entry{setup}{підготування для роботи}\\
\entry{share}{поши́рити}\\
\hypertarget{startup}{\entry{start-up}{розпочин}}\\
\entry{startup}{див. \hyperlink{startup}{start-up}}\\
\entry{store}{\textbf{1.} крамни́ця \textbf{2.} схо́вище}\\
\entry{support}{пі́дтримок}

\section{T}
\entry{tactical role-playing game}{тактична рольова гра}\\
\entry{tactical turn-based}{див. \hyperlink{tbt}{turn-based tactics}}\\
\entry{task}{завда́ння́}\\
\hypertarget{telephone}{\entry{telephone}{у́жва, слухавка, телефон}}\\
\entry{topic}{те́ма}\\
\entry{tripod}{трині́жок (-жка), (штатив) стоя́к}\\
\hypertarget{tbt}{\entry{turn-based tactics}{покрокова тактика}}

\end{twocolumn}

\end{document}
